...
def run_comparison():
    ...
    for tick in range(1, 61): 
        ...
        if getattr(controller, 'steam_voltage', 0.0) > 1.0:
            controller.states["sensor.hapsic_duct_rh"] += 2.0
        else:
            if controller.states["sensor.hapsic_duct_rh"] > 30.0:
                 controller.states["sensor.hapsic_duct_rh"] -= 0.5
        ...
        if tick % 2 == 0:
            print(f"Tick {tick:02d} | State: {controller.fsm_state:<13} | "
                  ...
                  f"Out: {controller.steam_voltage:.2f}V | IntA {controller.integrator_a:.2f} | IntB {controller.integrator_b:.2f} | Der: {controller.duct_derivative:.2f}")
